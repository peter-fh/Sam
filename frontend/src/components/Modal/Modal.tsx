...
function Modal() {
  ...
  const courseSelectModal = () => {
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md transition-all duration-300" data-testid="course-modal">
        <div className="w-full max-w-md bg-slate-900/90 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl backdrop-blur-xl flex flex-col items-center text-center animate-in fade-in zoom-in duration-200">
          ...
          <h2 className="text-xl font-bold text-slate-100 mb-2 no-underline">Select Your Course</h2>
          <p className="text-slate-400 text-sm mb-6">Choose your Concordia course so Sam can provide tailored assistance.</p>
          
          <div className="w-full mb-6">
            <select 
              className="w-full bg-slate-800/90 text-slate-100 border border-slate-700 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 transition-all cursor-pointer text-center font-medium shadow-sm" 
              onChange={onChange}
            > 
  ...
  const disclaimerModal = () => {
    return (
      <div id="DisclaimerModal" className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-slate-950/85 backdrop-blur-md overflow-y-auto" data-testid="disclaimer-modal">
        <div className="w-full max-w-2xl my-auto bg-slate-900/90 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl backdrop-blur-xl flex flex-col text-slate-200 animate-in fade-in zoom-in duration-200">
          <div className="flex items-center gap-3 mb-4 pb-4 border-b border-slate-800">
            <div className="w-10 h-10 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-400 flex items-center justify-center shrink-0">
              <i className="fa-solid fa-shield-halved text-lg"></i>
            </div>
            <div>
              <h2 className="text-xl font-bold text-slate-100 no-underline">Disclaimer & Ethical Use</h2>
              <p className="text-xs text-slate-400">Concordia Sam AI Tutoring Assistant</p>
            </div>
          </div>
          ...
          <div className="space-y-4 text-sm text-slate-300 leading-relaxed max-h-[60vh] overflow-y-auto pr-2">
            ...
            <ol className="list-decimal pl-5 space-y-3 marker:text-indigo-400 marker:font-semibold">
              <li className="pl-1">
                <span className="font-semibold text-slate-200">Accuracy:</span> Gen AI can make mistakes. It is the student's responsibility to double check any answers they are given.
              </li>
              <li className="pl-1">
                <span className="font-semibold text-slate-200">Academic Integrity:</span> Plagiarism, including submitting work that is generated or assisted by AI tools as your own without proper understanding or citation, is a violation of academic integrity policies. Always adhere to your professor's guidelines as per how and to what extent Gen AI can be used in your studies.
              </li>
            </ol>

            <div className="p-4 rounded-xl bg-indigo-500/10 border border-indigo-500/20 text-indigo-300 text-xs leading-normal">

...
from api.model import EXAMPLE_REVIEW_FILEPATH, ReviewerModel, TutorModel, EXAMPLE_RESPONSE_FILEPATH, estimateTokens
...
REVIEW_FILE_PATH = O3_DIR + os.sep + "review.md"

class OpenAI_o3_mini(TutorModel, ReviewerModel):
    debug: bool
    mock: bool
    ...
    def __init__(self, api_key: str, debug=False, mock=False):
        ...
        self.prompt_manager.setReview(REVIEW_FILE_PATH)
    ...
    def review(self, conversation, course_prompt):
        prompt = self.prompt_manager.reviewPrompt() + "\n" + course_prompt
        ...
        if self.mock:
            time.sleep(4)
            with open(EXAMPLE_REVIEW_FILEPATH) as f:
                time.sleep(2)
                for line in f:
                    for word in line.split(" "):
                        time.sleep(0.002)
